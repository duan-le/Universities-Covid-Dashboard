...
<body>
  ...
  <script>
    ...
    $sql = "SELECT * FROM manitoba WHERE university_name = 'University of Manitoba' AND cases != 0 ORDER BY STR_TO_DATE(date_range, '%e-%c-%Y') ASC";
    $casesData = array();
      ...
      <div class="col-auto">
        <table class="table table-striped table-bordered" id="data-table">
          ...
          <tbody>
            ...
            $sql = "SELECT * FROM manitoba WHERE university_name = 'University of Manitoba' AND cases != 0 ORDER BY STR_TO_DATE(date_range, '%e-%c-%Y') DESC";
            if ($result = mysqli_query($link, $sql)) {
              if (mysqli_num_rows($result) > 0) {
                while ($row = mysqli_fetch_array($result)) {
                  $date_arr = explode("-", $row['date_range']);
